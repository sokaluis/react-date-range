...
> **Versioning:** the version above tracks `package.json` as the source of truth. The current stable line is **1.3.x** after maintainer publish (see the `version` field in `package.json`).
