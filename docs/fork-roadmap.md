...
`1.0.0-rc.0` was validated (consumer spikes passed: React 18/19, SSR CJS+ESM, tree-shaking). `1.0.0` stable was published; the stable line is now `1.3.x` after maintainer publish (`v1.1.1` tag exists but was never promoted).
...
| **1.3.x — Configurable UI foundation** | Stable UI slots, `classNames`/`styles` hooks, opt-in CSS tokens, demo token fix | ✅ Complete and prepared as `v1.3.0` | `1.3.x` |
