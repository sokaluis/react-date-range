...
describe('opt-in design token stylesheet', () => {
  ...
  test('package exports the opt-in token stylesheet and packages it into dist', () => {
  ...
  test('maps selected UI to the primary token only when this stylesheet is imported', () => {
    ...
    expect(tokensCss).toMatch(/\.rdrSelected,\s*\.rdrInRange,\s*\.rdrStartEdge,\s*\.rdrEndEdge\s*{[^}]*color:\s*var\(--rdr-color-primary\)/s);
  });
